...
// ASSERT: scan joining\s*thread\s*has\s*received\s*the\s*InterruptedException\s*interrupt\s*status\s*has\s*been\s*cleared\,\s*if\s*the\s*output\s*is\s*false\s*\-\-\s*false