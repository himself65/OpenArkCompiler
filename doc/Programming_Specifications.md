...
<big><big><big><big><big><strong> 方舟编译器C++语言通用编程规范 </strong><big><big><big><big><big>
...
| [1 原则](#c1)        | [好代码的原则](#c1-1) [类和函数设计指导原则](#c1-2) [保证静态类型安全](#c1-3) [遵循C++ ISO标准](#c1-4) <br>[优先编译时检查错误](#c1-5) [使用命名空间来限定作用域](#c1-6) [优先使用C++特性而不是C特性](#c1-7) |
...
| [9 C++其他特性](#c9)   | [常量与初始化](#c9-1) [表达式](#c9-2) [类型转换](#c9-3) [资源分配和释放](#c9-4) [标准库](#c9-5) [const的用法](#c9-6) [异常](#c9-7) [模板](#c9-8) [宏](#c9-9) |
...
### <a name="a10-4-2"></a>建议10.4.2 在接口层面明确指针不会为`nullptr`
**理由**
1. 避免解引用空指针的错误。
2. 避免重复检查空指针，提高代码效率。

建议使用`gsl::not_null`，或参考实现自己的版本（如使用`NullObject模式`）。对于集合，在不违反已有的接口约定的情况下，建议返回空集合而避免返回空指针，当返回字符串时，建议返回空串`""`。
...
**注意**
`not_null`由`guideline support library(gsl)`提供。
